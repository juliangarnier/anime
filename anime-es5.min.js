...
a[b]))return"attribute";if(!d.null(a[b])&&!d.undef(a[b]))return"object"},y=function(a,b){return getComputedStyle(a).getPropertyValue(B(b))},S=function(a,b){var c=k(b,"scale")?1:0;a=a.style.transform;if(!a)return c;for(var e=/(\w+)\((.+?)\)/g,f=[],d=[],E=[];f=e.exec(a);)d.push(f[1]),E.push(f[2]);a=E.filter(function(a,c){return d[c]===b});return a.length?a[0]:c},H=function(a,b){var c=G(a,b);return"transform"==c?S(a,b):"css"==c?y(a,b):"attribute"==c?a.getAttribute(b):a[b]||0},I=function(a,b,c){if(d.color(b)){if(!d.rgb(b)&&
!d.rgba(b))if(d.hex(b))b=R(b);else if(d.hsl(b)){a=/hsl\((\d+),\s*([\d.]+)%,\s*([\d.]+)%\)/g.exec(b);b=parseInt(a[1])/360;c=parseInt(a[2])/100;a=parseInt(a[3])/100;if(0==c)c=a=b=a;else{var e=.5>a?a*(1+c):a+c-a*c,f=2*a-e;c=x(f,e,b+1/3);a=x(f,e,b);b=x(f,e,b-1/3)}b="rgb("+255*c+","+255*a+","+255*b+")"}else b=void 0;return b}if(l(b))return b;a=l(a.to)?l(a.to):l(a.from);!a&&c&&(a=l(c));return a?b+a:b},J=function(a){var b=/-?\d*\.?\d+/g;return{original:a,numbers:w(a).match(b)?w(a).match(b).map(Number):[0],
strings:w(a).split(b)}},T=function(a,b,c){return b.reduce(function(b,f,d){return b+a[d-1]+(f?f:c[d-1])})},U=function(a){return(a?u(d.array(a)?a.map(r):r(a)):[]).map(function(a,c){return{target:a,id:c}})},V=function(a,b){var c=[],e;for(e in a)if(!n.hasOwnProperty(e)&&"targets"!==e){var f=d.object(a[e])?v(a[e]):{value:a[e]};f.name=e;c.push(t(f,b))}return c},K=function(a,b,c,e){"transform"===c?(c=a+("("+F(a,b.from,b.to)+")"),b=a+("("+F(a,b.to)+")")):(a="css"===c?y(e,a):void 0,c=I(b,b.from,a),b=I(b,b.to,
a));return{from:J(c),to:J(b)}},W=function(a,b){var c=[];a.forEach(function(e,f){var m=e.target;return b.forEach(function(b){var k=G(m,b.name);if(k){var p;p=b.name;var g=b.value,g=r(d.func(g)?g(m,f):g);p={from:1<g.length?g[0]:H(m,p),to:1<g.length?g[1]:g[0]};g=v(b);g.animatables=e;g.type=k;g.from=K(b.name,p,g.type,m).from;g.to=K(b.name,p,g.type,m).to;g.round=d.color(p.from)||g.round?1:0;g.delay=(d.func(g.delay)?g.delay(m,f,a.length):g.delay)/h.speed;g.duration=(d.func(g.duration)?g.duration(m,f,a.length):
g.duration)/h.speed;c.push(g)}})});return c},X=function(a,b){a=W(a,b);return Q(a,["name","from","to","delay","duration"]).map(function(a){var b=v(a[0]);b.animatables=a.map(function(a){return a.animatables});b.totalDuration=b.delay+b.duration;return b})},z=function(a,b){a.tweens.forEach(function(c){var e=c.from,f=a.duration-(c.delay+c.duration);c.from=c.to;c.to=e;b&&(c.delay=f)});a.reversed=a.reversed?!1:!0},L=function(a){var b=[],c=[];a.tweens.forEach(function(a){if("css"===a.type||"transform"===
a.type)b.push("css"===a.type?B(a.name):"transform"),a.animatables.forEach(function(a){return c.push(a.target)})});return{properties:D(b).join(", "),elements:D(c)}},Y=function(a){var b=L(a);b.elements.forEach(function(a){return a.style.willChange=b.properties})},M=function(a){L(a).elements.forEach(function(a){return a.style.removeProperty("will-change")})},Z=function(a,b){var c=a.path,e=a.value*b,f=function(f){return c.getPointAtLength(1<b?a.value+(f||0):e+(f||0))},d=f(),h=f(-1),f=f(1),k=a.name;return"translateX"==
k?d.x:"translateY"==k?d.y:"rotate"==k?180*Math.atan2(f.y-h.y,f.x-h.x)/Math.PI:void 0},aa=function(a,b){var c=Math.min(Math.max(b-a.delay,0),a.duration)/a.duration;b=a.to.numbers.map(function(b,f){f=a.from.numbers[f];var d=A[a.easing](c,a.elasticity);b=a.path?Z(a,d):f+d*(b-f);return a.round?Math.round(b*a.round)/a.round:b});return T(b,a.to.strings,a.from.strings)},N=function(a,b){var c=void 0;a.time=Math.min(b,a.duration);a.progress=a.time/a.duration*100;a.tweens.forEach(function(a){a.currentValue=
