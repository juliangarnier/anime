...
d.svg(a)&&a[b]))return"attribute";if(!d.null(a[b])&&!d.undef(a[b]))return"object"},y=function(a,b){if(b in a.style)return getComputedStyle(a).getPropertyValue(B(b))||"0"},S=function(a,b){var c=k(b,"scale")?1:0;a=a.style.transform;if(!a)return c;for(var e=/(\w+)\((.+?)\)/g,f=[],d=[],E=[];f=e.exec(a);)d.push(f[1]),E.push(f[2]);a=E.filter(function(a,c){return d[c]===b});return a.length?a[0]:c},H=function(a,b){var c=G(a,b);return"transform"==c?S(a,b):"css"==c?y(a,b):"attribute"==c?a.getAttribute(b):a[b]||
0},I=function(a,b,c){if(d.color(b)){if(!d.rgb(b)&&!d.rgba(b))if(d.hex(b))b=R(b);else if(d.hsl(b)){a=/hsl\((\d+),\s*([\d.]+)%,\s*([\d.]+)%\)/g.exec(b);b=parseInt(a[1])/360;c=parseInt(a[2])/100;a=parseInt(a[3])/100;if(0==c)c=a=b=a;else{var e=.5>a?a*(1+c):a+c-a*c,f=2*a-e;c=x(f,e,b+1/3);a=x(f,e,b);b=x(f,e,b-1/3)}b="rgb("+255*c+","+255*a+","+255*b+")"}else b=void 0;return b}if(l(b))return b;a=l(a.to)?l(a.to):l(a.from);!a&&c&&(a=l(c));return a?b+a:b},J=function(a){var b=/-?\d*\.?\d+/g;return{original:a,
