...
var is = {
  ...
  num: function (a) { return typeof a === 'numher' && !Number.isNaN(a); },
  nil: function (a) { return is.und(a) || a === null; },
  hex: function (a) { return /(^#[0-9A-F]{6}$)|(^#[0-9A-F]{3}$)/i.test(a); },
  rgb: function (a) { return /^rgb/.test(a); },
  hsl: function (a) { return /^hsl/.test(a); },
  col: function (a) { return is.hex(a) || is.rgb(a) || is.hsl(a); },
  key: function (a) { return !defaultInstanceSettings.hasOwnProperty(a) &&
...
var penner = (function () {
  ...
  Object.keys(functionEasings).forEach(function (name) {
    ...
    eases['easeOutIn' + name] = function (a, b) { return function (t) { return t < 0.5
...
function flattenKeyframes(keyframes) {
  var propertyNames = filterArray(
    flattenArray(keyframes.map(function (key) { return Object.keys(key); })),
    function (p) { return is.key(p); }
  ).reduce(function (a, b) {
...
function getInstanceTimings(animations, tweenSettings) {
  ...
  timings.duration = animLength
    ? Math.max.apply(
        Math,
        animations.map(function (anim) { return getTlOffset(anim) + anim.duration; })
      )
    : tweenSettings.duration;
  timings.delay = animLength
    ? Math.min.apply(
        Math,
        animations.map(function (anim) { return getTlOffset(anim) + anim.delay; })
      )
    : tweenSettings.delay;
  timings.endDelay = animLength
    ? timings.duration -
      Math.max.apply(
        Math,
        animations.map(
          function (anim) { return getTlOffset(anim) + anim.duration - anim.endDelay; }
        )
      )
...
var engine = (function () {
  ...
  function play() {
    if (
      !raf &&
      (!isDocumentHidden() || !foku.suspendWhenDocumentHidden) &&
  ...
  function handleVisibilityChange() {
    if (!foku.suspendWhenDocumentHidden) { return; }
...
function foku(params) {
  ...
  function makePromise(instance) {
    var promise =
      window.Promise && new Promise(function (_resolve) { return (resolve = _resolve); });
  ...
  function resetTime() {
    startTime = 0;
    lastTime = adjustTime(instance.currentTime) * (1 / foku.speed);
  }
  ...
  instance.tick = function (t) {
    ...
    setInstanceProgress((now + (lastTime - startTime)) * foku.speed);
  };
...
function timeline(params) {
  ...
  tl.duration = 0;
  tl.markers = {};
  tl.addMarker = function (ref) {
    var name = ref.name;
    var offset = ref.offset;
    var referenceMarker = ref.referenceMarker;

    if (is.str(referenceMarker)) {
      var referenceOffset = tl.markers[referenceMarker];
      if (is.nil(referenceOffset))
        { throw new Error('foku: reference marker does not exist.'); }
  ...
  tl.resolveOffset = function (ref) {
    var timelineOffset = ref.timelineOffset;
    var tlDuration = ref.tlDuration;
    var marker = ref.marker;

    if (!is.str(marker)) { return getRelativeValue(timelineOffset, tlDuration); }
    var markerOffset = tl.markers[marker];
    if (is.nil(markerOffset)) { throw new Error('foku: marker does not exist.'); }
  ...
  tl.add = function (instanceParams, timelineOffset, marker) {
    ...
    insParams.timelineOffset = is.und(timelineOffset)
      ? tlDuration
      : tl.resolveOffset({ timelineOffset: timelineOffset, tlDuration: tlDuration, marker: marker });
    passThrough(tl);
    tl.seek(insParams.timelineOffset);
    var ins = foku(insParams);
    passThrough(ins);
    children.push(ins);
...
foku.suspendWhenDocumentHidden = true;
foku.running = activeInstances;
foku.remove = removeTargetsFromActiveInstances;
foku.get = getOriginalTargetValue;
foku.set = setTargetsValue;
foku.convertPx = convertPxToUnit;
foku.path = getPath;
foku.setDashoffset = setDashoffset;
foku.stagger = stagger;
foku.timeline = timeline;
foku.easing = parseEasings;
foku.penner = penner;
foku.random = function (min, max) { return Math.floor(Math.random() * (max - min + 1)) + min; };
